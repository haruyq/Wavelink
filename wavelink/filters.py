...
class EqualizerPresets:
    ...
    BASS_REDUCE: list[EqualizerPayload] = _parse_gains([
        -0.25, -0.22, -0.18, -0.14, -0.10, -0.06, -0.02, 0.00, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14
    ])
    VOICE_BOOST: list[EqualizerPayload] = _parse_gains([
        -0.10, -0.08, -0.06, -0.04, -0.02, 0.04, 0.08, 0.12, 0.16, 0.18, 0.16, 0.12, 0.08, 0.04, 0.00
    ])
    ROCK: list[EqualizerPayload] = _parse_gains([
        0.30, 0.25, 0.20, 0.10, 0.05, -0.05, -0.15, -0.20, -0.10, -0.05, 0.05, 0.10, 0.20, 0.25, 0.30
    ])
    POP: list[EqualizerPayload] = _parse_gains([
        0.26, 0.22, -0.21, -0.19, -0.16, 0.21, 0.22, 0.24, 0.24, 0.24, -0.05, -0.12, 0.19, 0.00, 0.00
    ])
    CLASSICAL: list[EqualizerPayload] = _parse_gains([
        0.38, 0.35, 0.13, 0.00, 0.00, 0.13, 0.55, 0.05, 0.13, 0.25, 0.20, 0.25, 0.30, 0.25, 0.30
    ])
    DEEP: list[EqualizerPayload] = _parse_gains([
        0.35, 0.30, 0.25, 0.20, 0.15, 0.10, 0.05, 0.00, -0.05, -0.10, -0.15, -0.20, -0.25, -0.25, -0.25
    ])
